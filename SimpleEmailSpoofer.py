...
def get_args():
    ...
    email_options.add_argument(
        "-t", "--to", dest="to_address", help="Email address to send to"
    )
    email_options.add_argument(
        "-a",
        "--to_address_filename",
        dest="to_address_filename",
        help="Filename containing a list of TO addresses",
    )
    email_options.add_argument(
        "-f", "--from", dest="from_address", help="Email address to send from"
    )
    email_options.add_argument("-n", "--from_name", dest="from_name", help="From name")

    email_options.add_argument(
        "-j", "--subject", dest="subject", help="Subject for the email"
    )
    email_options.add_argument(
        "-e",
        "--email_filename",
    ...
    email_options.add_argument(
        ...
        "--interactive",
        action="store_true",
        dest="interactive_email",
        help="Input email in interactive mode",
    )

    email_options.add_argument(
        "-r", "--reply-to", dest="reply_to", help="Set a reply-to header"
    )

    email_options.add_argument(
        "--image", action="store", dest="image", help="Attach an image"
    )
    email_options.add_argument(
        "--attach", action="store", dest="attachment_filename", help="Attach a file"
    )
    ...
    tracking_options.add_argument(
        "-d", "--db", dest="db_name", help="SQLite database to store GUIDs"
    )

    smtp_options = parser.add_argument_group("SMTP options")
    smtp_options.add_argument(
        "-s",
        "--server",
        dest="smtp_server",
        help="SMTP server IP or DNS name (default localhost)",
        default="localhost",
    )
    smtp_options.add_argument(
        "-p",
    ...
    smtp_options.add_argument(
        "--user", dest="smtp_user", help="SMTP username", default=None
    )
    smtp_options.add_argument(
        "--pass", dest="smtp_pass", help="SMTP password", default=None
    )
    smtp_options.add_argument(
        "--ssl", dest="smtp_ssl", help="Connect to SMTP server via SSL", default=False
    )
    smtp_options.add_argument(
        "--slow",
        action="store_true",
...
def is_domain_spoofable(from_address, to_address):
    ...
    if from_domain == "gmail.com":
        if to_domain == "gmail.com":
            output_bad(
                "You are trying to spoof from a gmail address to a gmail address."
            )
            output_bad(
                "The Gmail web application will display a warning message on your email."
            )
...
def save_tracking_uuid(email_address, target_uuid):
    global db
    db.execute(
        "INSERT INTO targets(email_address, uuid) VALUES (?, ?)",
        (email_address, target_uuid),
    )
    db.commit()
